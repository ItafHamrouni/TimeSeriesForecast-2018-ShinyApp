...
shinyServer(function(input,output,session){
  ...
  output$Plot1<-renderPlot({
    ...
    if(input$F2==1){
       ...
       lines(forecast1$fit,col="seagreen",lty=2)
       lines(OutsampleTs,col="red",lty=2)
     }
     if(input$F2==2){
  ...
  output$Plot2<-renderPlot({
    ...
    OutsampleTs<-window(TotalTS,start=c(Hstar(),1),end = c(Hend(),Fre()))
      ...
      if(input$F2==1){
        forecast1<-ses(InsampleTs,h=length(OutsampleTs),level = input$CI)
        barplot(accuracy(forecast1,OutsampleTs),legend=rownames(accuracy(forecast1,OutsampleTs)),main="Accuracy Test",beside=TRUE, col=c("palevioletred","tomato"))
      }
      if(input$F2==2){
        forecast1<-ses(InsampleTs,h=length(OutsampleTs),initial = "simple",level = input$CI,alpha = input$AlphaS)
        barplot(accuracy(forecast1,OutsampleTs),legend=rownames(accuracy(forecast1,OutsampleTs)),main="Accuracy Test",beside=TRUE, col=c("palevioletred","tomato"))
  ...
  output$Plot3<-renderPlot({
    ...
    OutsampleTs<-window(TotalTS,start=c(Hstar(),1),end = c(Hend(),Fre()))
      ...
      if(input$F4==1){
        ...
        lines(forecast$fit,col="seagreen",lty=2)
        lines(OutsampleTs,col="red",lty=2)
      }
      if(input$F4==2){
  ...
  output$Plot4<-renderPlot({
    ...
    OutsampleTs<-window(TotalTS,start=c(Hstar(),1),end = c(Hend(),Fre()))
      if(input$F4==1){
        ...
        barplot(accuracy(forecast2,OutsampleTs),legend=rownames(accuracy(forecast2,OutsampleTs)),main="Accuracy Test",beside=TRUE, col=c("palevioletred","tomato"))
      }
      if(input$F4==2){
    ...
    OutsampleTs<-window(TotalTS,start=c(Hstar(),1),end = c(Hend(),Fre()))
      ...
      if(input$F6==1){
        if(input$AM==1){
         ...
         plot(forecast3)
         lines(forecast3$fit,col="seagreen",lty=2)
         lines(OutsampleTs,col="red",lty=2)
        }
        if(input$AM==2){
  ...
  output$Plot6<-renderPlot({
    ...
    OutsampleTs<-window(TotalTS,start=c(Hstar(),1),end = c(Hend(),Fre()))
      ...
      if(input$F6==1){
        if(input$AM==1){
          forecast3<-hw(InsampleTs,h=length(OutsampleTs),level = input$CI2)
          barplot(accuracy(forecast3,OutsampleTs),legend=rownames(accuracy(forecast3,OutsampleTs)),main="Accuracy",beside=TRUE, col=c("palevioletred","tomato"))
        }
        if(input$AM==2){
          forecast3<-hw(InsampleTs,h=length(OutsampleTs),level = input$CI2,"multiplicative")
          barplot(accuracy(forecast3,OutsampleTs),legend=rownames(accuracy(forecast3,OutsampleTs)),main="Accuracy",beside=TRUE, col=c("palevioletred","tomato"))
  ...
  output$plot8<-renderPlot({
    ...
    pred<-predict(reg,New,interval = "prediction",level=input$CI3)
     ...
     if(input$Trans==2){
      FR<-exp(pred[,1])
      barplot(accuracy(FR,OutsampleTs),main="Outsample Accuracy of Forecasting",col="palevioletred3")
    }
    if(input$Trans==3){
      FR<-pred[,1]^2
      barplot(accuracy(FR,OutsampleTs),main="Outsample Accuracy of Forecasting",col="palevioletred3")
  ...
  output$Plot9<-renderPlot({
    ...
    lines(OutsampleTs,col="goldenrod1",lty=2)
  ...
  output$Plot10<-renderPlot({
    ...
    OutsampleTs<-window(TotalTS,start=c(Hstar(),1),end = c(Hend(),Fre()))
    forecast5<-forecast(nnetar(InsampleTs,level = input$CI4,h=length(OutsampleTs)))
    barplot(accuracy(forecast5,OutsampleTs),legend=rownames(accuracy(forecast5,OutsampleTs)),main="Accuracy",beside=TRUE, col=c("palevioletred","tomato"))
  ...
  output$Plot11<- renderPlot({
    ...
    lines(fitted(forecast6),col="seagreen",lty=2)
    lines(OutsampleTs,col="red",lty=2)
  ...
  output$Plot12 <- renderPlot({
    ...
    barplot(accuracy(forecast6,OutsampleTs),legend=rownames(accuracy(forecast6,OutsampleTs)),main="Accuracy",beside=TRUE, col=c("palevioletred","tomato"))
  ...
  output$Plot0<- renderPlot({
    ...
    lines(forecast0$fit,col="seagreen",lty=2)
    lines(OutsampleTs,col="red",lty=2)
  ...
  output$Plot00 <- renderPlot({
    ...
    OutsampleTs<-window(TotalTS,start=c(Hstar(),1),end = c(Hend(),Fre()))
    forecast0<-naive(InsampleTs,h=length(OutsampleTs))
    barplot(accuracy(forecast0,OutsampleTs),legend=rownames(accuracy(forecast0,OutsampleTs)),main="Accuracy",beside=TRUE, col=c("palevioletred","tomato"))
