...
tabPanel("Data Summary",
          sidebarLayout(
            ...
            sidebarPanel(
               radioButtons("RD1",label=h3(strong("Data Type")),choices = list("Default data"=1,"Your Data"=2),selected = 1),
               ...
               numericInput("End",strong("End Period"),value = 1961),
             ...
             tabPanel("Simple Exponential Smoothing",
                      sidebarLayout(
                        ...
                        mainPanel(
                          fluidRow(
                            h3('Model Introduction'),
                            p('Simple exponential smoothing. The simplest of the exponentially smoothing methods is naturally called simple exponential smoothing (SES). (In some books, it is called single exponential smoothing.) This method is suitable for forecasting data with no trend or seasonal pattern(1).',align='justify'),
             ...
             tabPanel("Linear Exponential Smoothing",
                      sidebarLayout(
                        ...
                        mainPanel(
                          fluidRow(
                            h3('Model Introduction'),
                            p('Holt (1957) extended simple exponential smoothing to allow forecasting of data with a trend. This method involves a forecast equation and two smoothing equations (one for the level and one for the trend)(2).',align='justify'),
             ...
             tabPanel("Holt Winter Method",
                      sidebarLayout(
                        ...
                        mainPanel(
                          fluidRow(
                            h3('Model Introduction'),
                            p('Holt (1957) and Winters (1960) extended Holt method to capture seasonality. The Holt Winters seasonal method comprises the forecast equation and three smoothing equations, one for the level, one for trend, and one for the seasonal component(3).',align='Justify'),
...
tabPanel("Regression Method ",
           sidebarLayout(
             sidebarPanel(
               radioButtons("Trans",h4(strong("Data Transformation")),choices = list("Normal"=1,"Logarithm"=2,"Square root"=3),selected = 1),
...
tabPanel("Neural Network Method",
           sidebarLayout(
           sidebarPanel(
...
tabPanel("Read Me",
           ...
           p("(1), (2), (3) from: Forecasting: principles and practice."),
           HTML('<Left><img src="predict.png" width="400" height="226"></Left>'),
           h3(strong("Author")),
           strong(p("Author: Itaf Hamrouni")),
           strong(p("Engineering in Statistics and Information Analysis")),
           strong(p("Email: itafhamrouni@gmail.com"))
